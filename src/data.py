def filter_articles(raw_df, start_date=None, end_date=None, verbose=False):
    """
    Filters the raw dataframe to only include rows that correspond to articles.

    Parameters:
    raw_df (pd.DataFrame): The raw dataframe containing all data, including articles and non-articles.
    start_date (str, optional): The start date for filtering articles. Defaults to None.
    end_date (str, optional): The end date for filtering articles. Defaults to None.


    Returns:
    pd.DataFrame: A filtered dataframe containing only article data.
    """
    # Filter out non-article
    art_df = raw_df[~raw_df["Post id"].isna()].reset_index(drop=True)

    if verbose:
        print(f"{art_df.shape[0]} articles from {raw_df.shape[0]} total rows.")
    ...
    if verbose:
        print(
            f"Filtered to {art_df.shape[0]} articles between {start_date} and {end_date}."
        )

    # Create user needs column
...
def scrape_articles():
    ...
    def flush_rows(rows: list[dict], path: str) -> None:
        ...
        df.to_csv(
            path, mode="a", index=False, header=write_header, encoding="utf-8-sig"
        )
...
def data_clean_and_save(raw_df, text_df, verbose=False, type=None) -> None:
    final_df = raw_df.merge(text_df, left_on="URL", right_on="url", how="inner")
    ...
    if type == "EDA":
        ...
        columns = [
            ...
            "Section",
            "User_Needs",
            # Some metrics (can be changed to include other metrics!)
            "Views",
            "Avg. views",
            "Engaged minutes",
            "Avg. minutes",
        ...
        if verbose:
            print(f"Final dataset shape: {eda_df.shape}")
            print(f"Columns: {eda_df.columns.tolist()}")

    if type == "ML":
        ml_df = final_df[["URL", "Title", "text", "User_Needs"]]
        ml_df = ml_df.dropna(ignore_index=True)
        ...
        if verbose:
            print(f"Final tagged dataset shape: {tagged_df.shape}")
            print(f"Final untagged dataset shape: {untagged_df.shape}")
            print(f"Columns: {tagged_df.columns.tolist()}")
